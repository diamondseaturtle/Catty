...
static int myprintf(const char *lpFormat, ...); //TODO: Do something about this mess omg
...
int SendReqWaitReply(int Sock, MessageHeader* Request, MessageHeader* &Response) {
	int Result = SUCCESS;
...
static unsigned int EchoThread(void* lpParameter) {
    ...
    // inbuf = (char *)malloc(g_Options.nBufSize);
    // outbuf = (char *)malloc(g_Options.nBufSize);
...
bool CreateConnectedSocket() {
	...
	//dude who the hell wrote this code initially lol
	//
	...
	fd_client = socket(addr_srv->ai_family, addr_srv->ai_socktype, addr_srv->ai_protocol);
	if (fd_client == INVALID_SOCKET ) {
		fprintf(stderr, "socket() failed: %d\n");
		return false;
	}

	res = connect(fd_client, addr_srv->ai_addr, addr_srv->ai_addrlen);
	if (res == SOCKET_ERROR ) {
